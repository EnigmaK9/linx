...
# Function to generate data for the comms_437_mhz_tx table
def generate_data(num_days):
    base_date = datetime.utcnow()
    for day in range(num_days):
        for _ in range(random.randint(1, 3)):  # Assuming between 1 and 3 downlink sessions per day
            start_hour = random.randint(0, 23)  # Downlink sessions can occur at any hour of the day
            ...
            power = random.uniform(5, 20)  # Power in watts, typical for a satellite transmitter
            ...
            cursor.execute('''
            insert into comms_437_mhz_tx (start_time, end_time, duration, power, voltage, priority_t, priority_e)
            values (?, ?, ?, ?, ?, ?, ?)
